...
class ChatsService {
  ...
  createRoom = async (userId, postId) => {
    const createChattingRoom = await this.chatsRepository.createRoom(
      userId,
      postId
    );
  ...
  createChats = async (message, userId, chatListId) => {
    const createChats = await this.chatsRepository.createChats(
      message,
      userId,
      chatListId
    );
  ...
  // 마지막 채팅 업데이트
  updateLastChat = async (chatListId, message) => {
    await this.chatsRepository.updateLastChat(chatListId, message);
  };
}
